package mrp_v2.customteleporters.config;
...
public class ServerConfig
{
    ...
    private static final String TRANSLATION_KEY = CustomTeleporters.ID + ".config";
    ...
    @SubscribeEvent public static void onLoad(final ModConfig.Loading configEvent)
    {
        LogManager.getLogger()
                .debug("Loaded " + CustomTeleporters.DISPLAY_NAME + " config file {}",
                        configEvent.getConfig().getFileName());
    }

    @SubscribeEvent public static void onFileChange(final ModConfig.Reloading configEvent)
    {
        LogManager.getLogger().debug(CustomTeleporters.DISPLAY_NAME + " config just got changed on the file system!");
    }
